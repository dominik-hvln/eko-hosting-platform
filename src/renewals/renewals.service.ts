...
export class RenewalsService {
  ...
  constructor(
    ...
    for (const service of servicesToRenew) {
      ...
      this.logger.log(
        `Checking service ${service.id} for user ${service.user.email}`,
      );
      ...
      if (priceString === null || priceString === undefined) {
        this.logger.warn(
          `Price for billing cycle "<span class="math-inline">\{service\.billingCycle\}" on plan "</span>{plan.name}" is not defined. Suspending service ${service.id}`,
        );
  ...
  private async suspendService(service: Service) {
    this.logger.warn(
      `Insufficient funds for service ${service.id}. Suspending.`,
    );
