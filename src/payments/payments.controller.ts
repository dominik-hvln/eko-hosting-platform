...
export class PaymentsController {
  ...
  createServiceRenewalSession(
    @Param('serviceId') serviceId: string,
    @Request() req,
  ) {
    return this.paymentsService.createServiceRenewalSession(
      req.user.userId,
      serviceId,
    );
  }
  ...
  async handlePayuWebhook(
    @Headers('openpayu-signature') signature: string,
    @Req() req: RequestWithRawBody,
  ) {
    // Puste na razie
  }
  ...
  createRequestPaymentSession(
    ...
    return this.paymentsService.createRequestPaymentSession(
      user.userId,
      requestId,
    );
  }
