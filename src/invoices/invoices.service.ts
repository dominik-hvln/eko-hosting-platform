...
export class InvoicesService {
  ...
  async createForTransaction(
    ...
    const invoice = this.invoicesRepository.create({
      ...
      seller: {
        name: 'EKO-HOSTING Sp. z o.o.',
        taxId: '123-456-78-90',
        addressLine1: 'ul. Wymyślona 1',
        zipCode: '00-000',
        city: 'Warszawa',
        country: 'Polska',
      },
  ...
  async generatePdf(invoice: Invoice): Promise<Buffer> {
    ...
    const formatCurrency = (amountInGr: number) =>
      (amountInGr / 100).toFixed(2);
    ...
    doc
      .fontSize(8)
      .font('Roboto')
      .text(`Miejsce wystawienia: Warszawa`, 400, 50, { align: 'right' });
    doc.text(`Data wystawienia: ${formatDate(invoice.issueDate)}`, {
      align: 'right',
    });
    doc.text(`Data sprzedaży: ${formatDate(invoice.saleDate)}`, {
      align: 'right',
    });
    ...
    doc.text('Wartość brutto', positions.wartoscBrutto, tableTop, {
      align: 'right',
    });
    drawLine(doc.y + 2);
    ...
    invoice.items.forEach((item, index) => {
      ...
      doc.text(formatCurrency(item.grossValue), positions.wartoscBrutto, y, {
        align: 'right',
      });
      doc.moveDown();
    });
    ...
    doc.text(
      `${formatCurrency(invoice.totalNetValue)} PLN`,
      summaryX_value,
      yPos,
      { align: 'right' },
    );
    ...
    doc.text(
      `${formatCurrency(invoice.totalGrossValue - invoice.totalNetValue)} PLN`,
      summaryX_value,
      yPos,
      { align: 'right' },
    );
