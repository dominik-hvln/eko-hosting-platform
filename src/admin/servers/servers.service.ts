...
export class ServersService {
  ...
  async queueServerProvisioning(serverId: string) {
    ...
    this.logger.log(
      `[PRODUCER] Zlecanie zadania 'provision-server' dla serwera ${serverId}`,
    );
    await this.provisioningQueue.add('provision-server', { serverId });
    return {
      message: `Zadanie provisioningu dla serwera ${serverId} zostało dodane do kolejki.`,
    };
  }
  ...
  async testConnection(
    ...
    try {
      ...
      if (result.code === 0) {
        ...
        throw new Error(
          result.stderr || `Komenda zakończyła się kodem błędu: ${result.code}`,
        );
      }
    } catch (error) {
      ssh.dispose();
  ...
  async findOne(id: string): Promise<Server> {
    ...
    if (server.sshPrivateKey) {
      try {
        server.sshPrivateKey = this.encryptionService.decrypt(
          server.sshPrivateKey,
        );
      } catch (e) {
        this.logger.error(
          `Could not decrypt private key for server ${id}. It might be corrupted or not encrypted.`,
        );
  ...
  async findOneForForm(id: string): Promise<Server> {
    ...
    try {
      ...
      this.logger.error(
        `Błąd deszyfrowania klucza dla serwera ${id}. Może być uszkodzony.`,
      );
